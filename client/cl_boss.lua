	...
	if open then 
		...
		while open do 
		   RageUI.IsVisible(MenuBossLTD,function()
                RageUI.Separator("↓     ~p~Gestion de l'entreprise     ~p~↓")
                RageUI.Button("Retirer argent de société", nil, {RightLabel = "~p~→→"}, true , {
                    onSelected = function()
                ...
                RageUI.Button("Déposer argent de société", nil, {RightLabel = "~p~→→"}, true , {
                    onSelected = function()
                        local amount = KeyboardInput("Montant", "", 10)
                            amount = tonumber(amount)
                            if amount == nil then
                ...
                RageUI.Button("Gestion Entreprise", nil, {RightLabel = "~p~→→→"}, true , {
                    onSelected = function()
                        aboss()
                        RageUI.CloseAll()
                    end
